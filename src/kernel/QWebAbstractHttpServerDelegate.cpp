...
#include "QWebAbstractHttpServerDelegate.h"
#include <QIODevice>
#include <QLocale>
#include <QTimer>
#include <QHttpHeader>
...
#include <QTextStream>
#include "QWebAbstractRessource.h"
#include "QWebAbstractRessourceProvider.h"
#include "QWebRessourceProviderServer.h"
#include "QWebMessageRessource.h"

QWebAbstractHttpServerDelegate::QWebAbstractHttpServerDelegate(QWebRessourceProviderServer* providerServer) :
    QThread(NULL),
    m_device(NULL),
    ...
    m_timer(NULL)
{
    ...
    m_notFoundRessource = new QWebMessageRessource();
    m_notFoundRessource->setMessage("<html><body><h2>Ressource not found</h2><br /><a href=\"http://qtweb.sourceforge.net\">QtWeb Application Server</a></body></html>");
    connect(this, SIGNAL(finished()), this, SLOT(deleteLater()));
}

QWebAbstractHttpServerDelegate::~QWebAbstractHttpServerDelegate()
{
    delete m_en_USLocale;
    delete m_notFoundRessource;
}

QIODevice* QWebAbstractHttpServerDelegate::device() const
{
    return m_device;
}

void QWebAbstractHttpServerDelegate::run()
{
    m_timer = new QTimer();
...
void QWebAbstractHttpServerDelegate::handleRequest(const QHttpRequestHeader& header)
{
...
void QWebAbstractHttpServerDelegate::handleHeadRequest(const QHttpRequestHeader& header)
{
    Q_UNUSED(header);
...
void QWebAbstractHttpServerDelegate::setSessionId(QHttpHeader& header, const QString& sessionId) const
{
    header.addValue("Set-Cookie", QString("QtWebSessionId=") + sessionId + "; Path=/; Version=1");
}

QString QWebAbstractHttpServerDelegate::sessionId(const QHttpHeader& header) const
{
    ...
    for (; it != itEnd; ++it) {
        int begin = it->indexOf("QtWebSessionId=");
        if (begin != -1) {
            begin += 15; // Length of "QtWebSessionId="
...
void QWebAbstractHttpServerDelegate::doRun()
{
    delete m_timer;
    m_timer = NULL;
